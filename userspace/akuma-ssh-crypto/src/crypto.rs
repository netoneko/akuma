...
/// Xorshift64 PRNG seeded from hardware entropy.
///
/// Callers provide a seed (typically from a hardware RNG). The kernel
/// wrapper creates one via `rng::fill_bytes`.
pub struct SimpleRng {
    state: u64,
}

impl SimpleRng {
    /// Create from an 8-byte seed. Use hardware entropy when available.
    #[must_use] 
    pub const fn from_seed(seed: [u8; 8]) -> Self {
        let s = u64::from_le_bytes(seed);
        Self { state: if s == 0 { 0xDEAD_BEEF_CAFE_BABE } else { s } }
    }
...
///
/// `rng` is used to generate random padding bytes.
pub fn build_encrypted_packet(
    ...
    // MAC(key, seq || unencrypted_packet) computed before encryption
