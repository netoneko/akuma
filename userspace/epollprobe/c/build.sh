#!/bin/bash
# Build `epoll_op_cost` and optionally push it into a running guest.
#
...
# both sides is the whole point — see the header of epoll_op_cost.c.
#
# Usage:
#   userspace/epollprobe/c/build.sh                    # just build
#   userspace/epollprobe/c/build.sh --push-akuma 2322  # + push over SSH
#   userspace/epollprobe/c/build.sh --push-lima  fc    # + push to a Lima VM
set -euo pipefail
cd "$(dirname "$0")"
OUT=epoll_op_cost

aarch64-linux-musl-gcc -static -O2 -Wall -Wextra -o "$OUT" epoll_op_cost.c
