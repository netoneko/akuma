...
| `stdinedge` | Same question as `ptyedge`, but against the real `FileDescriptor::Stdin` exec-channel on fd 0 (Akuma has no real pty subsystem — `ptyedge` can't reach the code path that actually matters). Needs a real interactive session and a keypress. |
| `bigspawn [N]` | Spawns the real, heavy rustc invocation cargo uses for `proc-macro2`'s build script (piped stdio, matching cargo's JSON-diagnostics capture) via plain `std::process::Command`, `N` times (default 50). Written to test whether cargo's `Bad address (os error 14)` spawn failure (`docs/archive/NCA_MISSING_SYSCALLS.md` §1) reproduces without cargo in the loop — as of 2026-08-18 it does not (50/50 clean), narrowing the bug to something cargo does concurrently with the spawn (jobserver, background threads), not the spawn shape itself. |
