...
async fn handle_publickey_auth(
    ...
    *offset += 1;
    ...
    if algorithm != b"ssh-ed25519" {
        println("[SSH Auth] Unsupported key algorithm");
        return (AuthResult::Failure, build_failure_response());
    }
    ...
    let client_key = match parse_key_blob(key_blob) {
        Some(k) => k,
        None => {
            println("[SSH Auth] Failed to parse client public key");
            return (AuthResult::Failure, build_failure_response());
        }
    };
    ...
    if !is_authorized {
        println("[SSH Auth] Key not in authorized_keys");
        return (AuthResult::Failure, build_failure_response());
    }
    ...
    let signature = match parse_signature_blob(signature_blob) {
        Some(s) => s,
        None => {
            println("[SSH Auth] Failed to parse signature");
            return (AuthResult::Failure, build_failure_response());
        }
    };
    ...
    match client_key.verify(&signed_data, &signature) {
        Ok(()) => {
            println("[SSH Auth] Signature verified successfully");
