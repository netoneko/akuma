...
fn cross_ar() -> &'static str {
    // Prefer llvm-ar from Homebrew, then the musl cross-archiver.
    // The macOS system `libtool` cannot handle ELF objects.
    let homebrew = "/opt/homebrew/opt/llvm/bin/llvm-ar";
    if Path::new(homebrew).exists() { homebrew } else { "aarch64-linux-musl-ar" }
}

fn cross_ranlib() -> &'static str {
    let homebrew = "/opt/homebrew/opt/llvm/bin/llvm-ranlib";
    if Path::new(homebrew).exists() { homebrew } else { "aarch64-linux-musl-ranlib" }
}
...
fn build_zstd(vendor_dir: &Path, deps_dir: &Path, cc: &str, gmake: &str) {
    ...
    // Build and install only the static library (BUILD_SHARED=no avoids macOS dylib flags)
    ...
    let status = Command::new(gmake)
        ...
        .arg(format!("PREFIX={}", deps_dir.display()))
        .status()
        .expect("Failed to build zstd");
    if !status.success() { panic!("zstd build failed"); }
}
...
fn build_xbps(xbps_src: &Path, deps_dir: &Path, staging_dir: &Path, cc: &str, gmake: &str) {
    ...
    // xbps configure uses pkg-config exclusively to find libarchive, libssl, zlib.
    // Point it ONLY at our cross-compiled deps so it doesn't pick up system libs.
    let pkgconfig_dir = deps_dir.join("lib/pkgconfig");

    let ldflags = format!("-static -L{} -Wl,--entry=_start", deps_dir.join("lib").display());
    let cflags = format!("-Os -I{}", deps_dir.join("include").display());
    ...
    let status = Command::new("./configure")
        ...
        .env("CFLAGS", &cflags)
        .env("LDFLAGS", &ldflags)
        // Override ALL pkg-config search paths so only our deps are found
        .env("PKG_CONFIG_LIBDIR", &pkgconfig_dir)
        .env("PKG_CONFIG_PATH", "")
        .arg("--prefix=/usr")
        .arg(format!("--build={}", host_triplet()))
        .arg("--host=aarch64-linux-musl")
        .arg("--sysconfdir=/etc")
        .arg("--enable-static")   // Build static xbps-* binaries
        .status()
        .expect("Failed to configure xbps");
    if !status.success() { panic!("xbps configure failed"); }

    // Patch generated Makefiles: xbps configure writes LDFLAGS into config.mk
    // and can override our flags. Patch to preserve -static and --entry=_start.
    patch_ldflags_in_config(xbps_src, &ldflags);
...
fn patch_ldflags_in_config(dir: &Path, ldflags: &str) {
    // xbps configure writes flags into config.mk
    for rel in &["config.mk", "Makefile", "lib/Makefile", "bin/Makefile"] {
        ...
        let patched: String = content
            ...
            .join("\n");
        if patched != content {
            println!("cargo:warning=Patched LDFLAGS in {}", rel);
            fs::write(&path, patched).expect("Failed to patch config file");
        }
    }
}
