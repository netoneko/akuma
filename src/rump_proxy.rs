...
impl BoxProxy {
    ...
    ///
    /// The sysproxy request is written to the kernel→server pipe as a SINGLE
    /// frame (see `Client::syscall`), so `pipe_write` wakes the server's poller
    /// exactly once, with the complete request — the client thread can never be
    /// preempted mid-request onto a partial frame (the old ~48 ms/leg stall).
    /// The reply-side block already hands off directly via `schedule_blocking`'s
    /// voluntary reschedule. So no preemption guard is needed here.
