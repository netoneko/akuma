#!/bin/bash
# One A/B/A arm for the epoll family: build the tree as it stands, boot it, gate
# on the epoll correctness probe, then take timing rounds. Written as a script
# because an A/B arm that is typed by hand twice is two different experiments.
#
#   scripts/benchmarks/epoll_ab_run.sh <label> <outdir> [smp] [rounds]
#
...
# and `epoll_op_cost` emits the same line format for exactly this reason. A
...
scripts/epoll_suite.py --port "$PORT" > "$OUT/suite.$LABEL.log" 2>&1 && SUITE=PASS || SUITE=FAIL
grep -E '^=====' "$OUT/suite.$LABEL.log" || true
echo "[arm $LABEL] suite: $SUITE"

echo "[arm $LABEL] timing"
userspace/epollprobe/c/build.sh > /dev/null
python3 - "$PORT" <<'PY'
import base64, subprocess, sys
b = open("userspace/epollprobe/c/epoll_op_cost", "rb").read()
subprocess.run(["ssh", "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null",
                "-o", "LogLevel=ERROR", "-p", sys.argv[1], "root@localhost",
                "base64 -d > /tmp/epoll_op_cost && chmod +x /tmp/epoll_op_cost"],
               input=base64.b64encode(b), capture_output=True)
PY
scripts/benchmarks/futex_op_ab.py --port "$PORT" --exe /tmp/epoll_op_cost \
    --rounds "$ROUNDS" --label "$LABEL" --save "$OUT/epoll.$LABEL.json"
