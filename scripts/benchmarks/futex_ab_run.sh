#!/bin/bash
# One A/B/A arm: build the tree as it stands, boot it, gate on the futex
# correctness probes, then take timing rounds. Written as a script because an
# A/B arm that is typed by hand twice is two different experiments.
#
#   scripts/benchmarks/futex_ab_run.sh <label> <outdir> [smp] [rounds]
#
...
echo "[arm $LABEL] correctness probes"
scripts/futex_suite.py --port "$PORT" > "$OUT/suite.$LABEL.log" 2>&1 && SUITE=PASS || SUITE=FAIL
tail -1 "$OUT/suite.$LABEL.log"
echo "[arm $LABEL] suite: $SUITE"

echo "[arm $LABEL] timing"
userspace/futexprobe/c/build.sh > /dev/null
python3 - "$PORT" <<'PY'
import base64, subprocess, sys
b = open("userspace/futexprobe/c/futex_op_cost", "rb").read()
subprocess.run(["ssh", "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null",
                "-o", "LogLevel=ERROR", "-p", sys.argv[1], "root@localhost",
                "base64 -d > /tmp/futex_op_cost && chmod +x /tmp/futex_op_cost"],
               input=base64.b64encode(b), capture_output=True)
PY
scripts/benchmarks/futex_op_ab.py --port "$PORT" --exe /tmp/futex_op_cost \
    --rounds "$ROUNDS" --label "$LABEL" --save "$OUT/futex.$LABEL.json"
