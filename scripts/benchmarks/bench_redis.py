...
# Usage
    ...
    bench_redis.py --label akuma-local  --via ssh:2222     --out akuma_local.json
...
def wrap(via, argv):
    ...
    sys.exit(f"unknown --via {via!r} (want host, ssh:PORT, or docker:NAME)")
...
def bench(via, host, port, requests, clients, size, pipeline):
    rc, out, err = run(via, ["redis-benchmark", "-h", host, "-p", str(port),
...
def main():
    ...
    rc, out, err = run(args.via, ["redis-cli", "-h", args.host, "-p", str(args.port),
    ...
    meta = {"label": args.label, "via": args.via, "target": f"{args.host}:{args.port}",
            ...
            "repeats": args.repeats, "when": time.strftime("%Y-%m-%d %H:%M:%S")}
    ...
    for p in [int(x) for x in args.pipelines.split(",")]:
        for r in range(1, args.repeats + 1):
            ...
            for t, v in bench(args.via, args.host, args.port, args.requests,
                              args.clients, args.size, p).items():
