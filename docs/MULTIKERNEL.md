...
| **R4b — per-core syscalls + pinned process** (staged R4b.1–.5) | SVC from EL0 on the per-core vectors; spawn `/bin/hello` (then `curl`) pinned to core 1, forwarding `open`/`read`/`write`/sockets to core 0 (§10) | The §10 acceptance test, staged like R1–R4a. **R4b.1 ✅** scheduler as steady state. **R4b.2 ✅** persistent BSP forward-server thread. **R4b.3** EL0 on the secondary — **R4b.3a ✅** the user-table kernel-window overlay (a user address space on a secondary resolves kernel statics to ITS private pages, verified); **R4b.3b** wire `sync_el0_handler` + syscall table → replicated state and run a local-only pinned process. **R4b.4** forwarded exec — `/bin/hello`, `open`/`read` forwarded to fetch the ELF (§10 Part A). **R4b.5** sockets — `curl` (§10 Part B). |
