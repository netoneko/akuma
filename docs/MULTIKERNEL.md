...
- **Networking + the full demo (§10 Part B, M3, acceptance/12) — R4b.5 DONE (SMP=2 @ 2 GB):**
  `curl https://ifconfig.me` runs PINNED to **core 1** and prints the box's real public IP
  (`87.71.63.90`). core 1 has no net and no VFS, so EVERY external dependency is forwarded to core 0
  (the Net + VFS owner) over the ring + bounce: **DNS** (a UDP socket + `sendto`/`recvmsg`),
  the **HTTPS** connection (`socket`/`connect`/`send`/`recv` on TCP), the **CA bundle** read
  (`/etc/ssl/certs/ca-certificates.crt`, 189 KB streamed in 4 KiB chunks), the curl **ELF** load,
  **entropy** (`/dev/urandom`/`getrandom` → core 0's virtio-rng, a BSP-only device), and the **wall
  clock** (seeded once from core 0's RTC so TLS cert date checks see real time, not 1970). Routing is
  per-fd: a forwarded `openat`/`socket` returns a `RemoteFd { owner, handle, kind }` and
  `read`/`write`/`close`/`recvmsg`/… on it forward, while local fds (pipes, `/dev`, console) stay
  local. The whole command line (program + args) rides the `core_init` activation so `curl` gets its
  `-sS https://ifconfig.me`; its stdout drains to core 0's UART via the §8.2 console ring. The owner
  side is non-blocking (the secondary retries on `-EAGAIN`) so the single BSP forward-server thread
...
(criterion 2, `curl`) is DONE (2026-07-01)** — `curl https://ifconfig.me` pinned to core 1 completes a
real HTTPS fetch and prints the public IP, with every socket syscall (DNS UDP + the TLS TCP
connection) serviced by core 0's stack, proving criterion 2 and (again) criterion 3 — the per-core
`PMM`/`POOL`/`TALC` stay physically isolated throughout. See the R4b.5 row in §15.
...
| **M3 — roles + forwarding** | The §10 acceptance test: `hello` + `curl` running on core 1 (spawned locally by core 1's own kernel) | **Part A ✅** — `/bin/hello` runs pinned on core 1: `core_init(idx, path)` activation (§6.1 — NOT a spawn message; the program path rides the `init_program` descriptor slot), VFS-read forwarding (the loader fetches the ELF over the R4a transport), local spawn via the normal loader + the `prepare_user_address_space` overlay hook, console append ring (§8.2) for its output, spawn/exit local to core 1's kernel. **Part B ✅** — `curl https://ifconfig.me` runs pinned on core 1 and prints the real public IP: socket arms (`socket`/`bind`/`connect`/`sendto`/`recvfrom`/`recvmsg`/`sendmsg`/`setsockopt`/`getsockopt`) on the owner-side forwarder, per-fd `RemoteFd` routing for read/write/close, DNS over forwarded UDP, forwarded entropy (`/dev/urandom`→core 0 virtio-rng) and a clock seeded from core 0's RTC. |
