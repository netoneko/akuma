...
| M5b — BKL-free user page-fault path (per-AS `as_lock`) | 🚧 Stages 1–3 + 4a done (primitives + syscall wiring + fault PTE edits under `as_lock` + file read/install split + **file-fault block I/O runs BKL-dropped**; SMP=2/4 verified). Self-test contention is scheduler/spawn-bound, not fault-bound, so it doesn't measure this win. Stage 4b (reconcile-aware full flip) scope under review |
