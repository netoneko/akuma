...
production lines it is the largest crate in the tree**, and it now reads **0
`unsafe` sites, 100.0% safe**. Group 3 (the two decoupled-pointer sites) turned
out to be non-problems: `fork`'s `demote_range_to_ro` `parent_l0`/`parent_as`
were only ever "independent" in the test — in production the live TTBR0 *is* the
leader's AS, so the raw `*const u64` param went away and the demote is a
`&mut self` method on `UserAddressSpace` (`akuma-mmu` +1); the vfork-prefault
"lock A, record in B" edge shares one L0 between A and B, so the special case
was deleted and the page tracks against the leader. **`akuma-exec` 2 → 0**;
`akuma-mmu` 70 → 71. **32 of 52 crates** forbid.

The run before it, the same day, was
`python3 scripts/cloc_akuma.py src crates` after Phase 7f groups 2a+2b
([`AKUMA_EXEC_AUDIT.md`](../archive/AKUMA_EXEC_AUDIT.md) §6.E): the
`execve`/first-run destructive window stopped needing a `&mut Process`
synthesised from a raw pointer. `Process::{state,exited,exit_code,entry_point,
initial_brk,process_info_phys,clear_child_tid,sigaltstack_*}` and the five
`ProcessMemory` scalars became atomics (the §5a call); `name`/`args`/`context`
moved behind one `Spinlock<ProcessImage>`; `replace_image` and `Process::run`
take `&self`. `table::{get_process_ptr,with_process_exclusive}` and
`SlotTable::active_exclusive` are gone. **`akuma-exec` 7 → 2** `unsafe` sites;
`akuma-slot-table` **11 → 9**.

The run before it, earlier the same day,
`python3 scripts/cloc_akuma.py src crates` after `akuma-slot-table` was carved
out of `akuma_exec::process::table` (§6.D). The lock-free `*mut Process` slot
store — the FREE/ACTIVE/RETIRED state array, the reuse generation, the retire
timestamp, and every deref — became a generic `SlotTable<T, N>` primitive that
never names `Process`, the move `akuma-locks-rw-cell`/`akuma-gic` made.
`akuma-exec` **25 → 7** `unsafe` sites (the identity cache's
`own_ptr`/`tgid_ptr` fields went too — `identity_get` derives the pointer from
`SlotTable::ref_if_current(slot, generation)`); the new `akuma-slot-table`
carried **11**, behind one stated contract.
...
not relocated, and no crate was added. `src/`'s **test** `unsafe` fell **77 ->
70** in the same run. **23 of 39 crates** forbid, unchanged.

Only **one** of `src/`'s three remaining sites is a real operation
(`akuma_fdt::locate`); the other two are `unsafe extern "C"` declaration blocks.
Of the 70 test sites, the reductions were six repeats of the same waker/`Pin`
defect plus one `msr fpcr` that became `akuma_cpu::sysreg::set_fpcr` — **the rest
are the tests' subject matter and should stay**
([`SRC_BOOT_ENTRY_UNSAFE_CLEANUP.md`](../archive/SRC_BOOT_ENTRY_UNSAFE_CLEANUP.md)
§6, which also records why a `crates/akuma-tests` split does not follow from
them).
...
| crate | what it holds |
|---|---|
| `akuma-exec` | the process table, `fork`/`vfork`/`clone`/`execve`, the `Process` lifecycle, fds, signals, channels, CoW-fork region algebra — **9,311 production lines, the largest crate in the tree**. Every `unsafe` operation it once carried moved to a crate that owns what it pokes ([`AKUMA_EXEC_AUDIT.md`](../archive/AKUMA_EXEC_AUDIT.md) §6). Re-exports `akuma-threading`, `akuma-el0-entry`, `akuma-user-access`, `akuma-mmu`, `akuma-slot-table` so its ~500 downstream call sites are unchanged |
| `akuma-exec-core` | the unsafe-free half: `Process`/`Thread` data types, `UserContext`, `ProcessState` (+ `AtomicProcessState`), stack-sizing policy |
...
Generated by `scripts/cloc_akuma.py src crates` (2026-09-02, after `akuma-exec`
took `#![forbid(unsafe_code)]` — [`AKUMA_EXEC_AUDIT.md`](../archive/AKUMA_EXEC_AUDIT.md) §6):

| | |
|---|---|
| enforced unsafe-free crates | **32 of 52** |
| code in those crates | **50,743 of 68,675** lines under `crates/` (73.9%) |
| `unsafe` sites across `crates/` | **384** (374 production, 10 test), of which **0** are inside an enforced crate |

The **62.1% -> 73.9%** step is `akuma-exec` alone: at 5,505 production lines it
is the single largest crate to make the move, and the campaign that got it
there (`AKUMA_EXEC_AUDIT.md` §6, ~10 extraction docs) is the largest single
piece of this table's history. `akuma-exec-core` (its unsafe-free data-type
half) was already enforced and is now listed too.

The earlier jump from **23 of 39 / 51.6%** came from three crates carved out of the
binary the same day — `akuma-syscalls-log`, `akuma-syscalls-ipc` and
`akuma-vfs-glue` — all three `#![forbid(unsafe_code)]`, none of them adding an
`unsafe` site anywhere, joined later the same day by
[`akuma-config`](../archive/AKUMA_CONFIG_EXTRACTION.md): `src/config.rs`'s 1,216
lines of tunables, minus 14 dead consts that a module-level
`#![allow(dead_code)]` had been hiding since the built-in SSH server was removed.
It forbids `unsafe` too — it is nothing but `const`s.

The 54.0% -> **62.1%** step is
[`akuma-syscalls-glue`](../archive/AKUMA_SYSCALLS_GLUE_EXTRACTION.md): ~17,000
lines of syscall dispatcher and family implementations, the largest thing that
was left in the binary, all of it already `#![forbid(unsafe_code)]` as a module
attribute and now enforced as a crate. It cost seven function pointers and made
`extreme-size` 4,088 bytes **smaller**. `src/` production `unsafe` did not move —
it was 3 before and after, because none of what left held any. They moved to break the `src/syscall/` ↔ `src/vfs/`
dependency cycle rather than to localise anything unsafe; the ratio improving is
a side effect of ~3,300 lines crossing from "bin" to "enforced crate". The `src`
row is unchanged at **3**, because none of what moved held any.
[`SRC_SYSCALL_EXTRACTION.md`](../archive/SRC_SYSCALL_EXTRACTION.md) §7.

**That 423 is right, and has been independently checked.** It was challenged as
"wildly inaccurate" against a `grep -c unsafe`, which reads 593 lines under
`crates/`; ~170 of those are comments (this codebase writes *about* `unsafe`
constantly) and 10 are `#[unsafe(…)]` attributes the script excludes on purpose,
because they mark a declaration rather than an operation. A separately written
lexer lands on 423 exactly. The 10 test sites are all in inline `#[cfg(test)] mod`
blocks — no `*_tests.rs` file under `crates/` contains any `unsafe` at all, which
is why that column looks so much smaller than `src/`'s 70.

`scripts/cloc_akuma.py --self-test` now carries **24 lexer cases and 6 structural
invariants** covering that reasoning, including "every crate under `crates/`
appears in the report" — the failure this table has actually had, when
`akuma-gic` and `akuma-psci` went missing from it. Writing the cases found one
real bug: `r#unsafe`, the raw-identifier escape and the one spelling of those six
letters that is *not* the keyword, was being counted as a site.
...
| `akuma-mmu` | 71 | page tables, `UserAddressSpace`, ASIDs, the per-core TTBR free gate. Was 64; the `oof-part-2` branch moved ~6 raw-pointer sites in as safe helpers for `akuma-exec` (`write_phys`, `read_current_user_val`, `with_phys_bytes_mut`, `write_current_user_val`, and 2026-09-02 `UserAddressSpace::demote_range_to_ro` — the `&self`-safe form of the free `demote_range_to_ro(*mut u64, …)` `unsafe fn`, walking `self.l0_frame`, so `akuma-exec`'s CoW-fork path holds no `unsafe`). Each is stated once here instead of at every call site |
