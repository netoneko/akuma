...
| | |
|---|---|
| enforced unsafe-free crates | **22 of 32** |
| code in those crates | **25,067 of 44,617** lines under `crates/` (56.2%) |
| `unsafe` sites across `crates/` | **304** (293 production), of which **0** are inside an enforced crate |

`cloc_akuma.py` also reports a second, different safety number: **96.6% of
...
The previous run (2026-08-30) read `crates/` 330/319/11 and `src/` 315/199/116,
tree **645/518/127**. The drop is the `akuma-cpu` migration: 183 `asm!` sites
that each carried a trivially-dischargeable `unsafe` block became safe calls.
...
density is **8.6 sites per kloc** of production code (was 9.8).
