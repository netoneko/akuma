...
| | |
|---|---|
| enforced unsafe-free crates | **27 of 43** |
| code in those crates | **28,445 of 52,664** lines under `crates/` (54.0%) |
...
It forbids `unsafe` too — it is nothing but `const`s. They moved to break the `src/syscall/` ↔ `src/vfs/`
