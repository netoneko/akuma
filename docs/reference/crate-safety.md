...
| `akuma-mmu` | 65 | page tables, `UserAddressSpace`, ASIDs, the per-core TTBR free gate. Gained `boot_device_l3_phys` (the boot-table walk that let `rebuild_boot_device_table` stop being `unsafe` for its callers) and, 2026-09-02, `UserAddressSpace::demote_range_to_ro` — the `&self`-safe form of the free `demote_range_to_ro(*mut u64, …)` `unsafe fn`, walking `self.l0_frame`, so `akuma-exec`'s CoW-fork path holds no `unsafe` (§6.E group 3) |
