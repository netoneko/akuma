...
Fourteen of the twenty-three extracted crates are **unsafe-free and enforced so**. Each
...
| | |
|---|---|
| enforced unsafe-free crates | **14 of 23** |
...
**10.5 sites per kloc of production code**.

**One crate is production-unsafe-free but not marked.** All 12 of
`akuma-syscalls-linux`'s sites are `transmute` layout assertions inside `#[test]`
bodies; its production half has none. It cannot take a plain
`#![forbid(unsafe_code)]` — that would ban the assertions too — but
`#![cfg_attr(not(test), forbid(unsafe_code))]` would enforce the property that
actually matters and take the count to 14 of 22. Not done; recorded here so the
