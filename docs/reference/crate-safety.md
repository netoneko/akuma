...
[`AKUMA_FDT_EXTRACTION.md`](../archive/AKUMA_FDT_EXTRACTION.md)):

| | |
|---|---|
| enforced unsafe-free crates | **22 of 35** |
| code in those crates | **25,194 of 45,709** lines under `crates/` (55.1%) |
| `unsafe` sites across `crates/` | **335** (324 production), of which **0** are inside an enforced crate |
...
Regenerated 2026-09-01 (`AKUMA_FDT_EXTRACTION.md`). `src/` production 113 ->
...
**Regenerated 2026-09-01** (`AKUMA_FDT_EXTRACTION.md`). The whole `sites` column
is from that one run, not just the new row: it had last been regenerated
2026-08-30 and five of its figures had drifted since — `akuma-cpu` 19 -> 32 and
`akuma-primitives` 14 -> 4 in opposite directions, which is exactly the pattern a
hand-maintained column hides. Sorted by site count, so the shape of the list is
visible: it is a short head of genuinely hardware-facing crates and a long tail
of crates holding one obligation each.
