...
half landed as `akuma-locks-rw-cell`. **22 of 33 crates** forbid (the counter's figure; `akuma-alloc` joined 2026-08-31 as a non-forbidding crate, so the denominator moved and the numerator did not). The three
...
**Twenty-two of the thirty-three** extracted crates are **unsafe-free and enforced so** (`enforced unsafe-free ... 22 of 33 crates`, straight from the counter). Each
...
| | |
|---|---|
| enforced unsafe-free crates | **22 of 33** |
...
### The one enforced subtree in `src/`
