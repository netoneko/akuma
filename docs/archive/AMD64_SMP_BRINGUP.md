...
is at `0x7fff_ffaf`, above `PHYSMAP_LIMIT` (1 GiB), so `machine::Physmap::read`
refused it and the MADT was invisible: `acpi: tables:` printed empty and the
kernel would have believed itself single-core on any 4-vCPU guest with more
than a gigabyte. `boot.s` has mapped 4 GiB since the bare-metal framebuffer
work; `phys::PHYSMAP_MAPPED` (4 GiB) is now the read bound, `PHYSMAP_LIMIT`
stays the PMM's. Firecracker's tables are at `0xA00xx` and never saw this.
...
| `amd64/src/phys.rs` | `PHYSMAP_MAPPED` |
