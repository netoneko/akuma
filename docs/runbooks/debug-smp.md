...
| `forktest_parent` hangs at "Launching child 0", kernel alive (0 `[BKL] stuck`), parent stuck in a ~60 s `futex` | **A regression on `another-smp-attempt-0` (confirmed: same `devbox.img` runs forktest cleanly on `main`'s kernel, hangs on the branch kernel — at every SMP incl. SMP=1).** Children close their pipes with `write_count=0` (log `[pipe] close_write … write_count=0`) — they exit without the parent seeing it; the parent's Go runtime parks on an `epoll` that never signals. Suspect a branch kernel delta breaking `EPOLLHUP` on pipe-write-end close, or child exit/wait notification. | Bisect the branch kernel commits (main→HEAD) against forktest. `rustc hello.rs` works on the branch, so fork/exec/mmap are fine — narrow to pipe/epoll/futex/wait paths. SMP=1-on-branch is NOT a valid "not-SMP" signal (branch kernel changes are compiled in regardless of core count). |
| `ssh` gets "connection reset / kex_exchange_identification" but kernel heartbeats continue | `/bin/sshd` `SIGSEGV`'d — grep the boot log for `[Fault] Process N (/bin/sshd) SIGSEGV`. Intermittent under SMP=4; kernel survives (userspace fault). | Suspected residual fork/CoW-under-SMP race on sshd's per-connection fork (cf. `fork_cow_tlb_asid_flush`). Reboot to recover; reproduce under `GDB=1` to catch the faulting child. |
