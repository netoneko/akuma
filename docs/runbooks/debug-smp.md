...
| Forked children (`/bin/sshd`, `/bin/busybox`, shells) `SIGSEGV` under SMP=4; `ssh` sees "connection reset / kex_exchange_identification" but kernel heartbeats continue | **A fork/clone context race under SMP=4 concurrency** (userspace fault, kernel survives). **Reliably reproduced** by fork-hammering (many concurrent SSH conns each forking a shell) — crashes right after `[FORK-COW] shared N pages`. Signature: `[WILD-DA]` on a CLONE_VM thread with `FAR=0`, `x0..x19` zeroed but `x20/x29/x30/SP/TPIDR` intact (partially-zeroed register context) → null-deref; also seen as an `ISS=0x7` translation fault. **NOT a missing TLB flush** — `flush_tlb_all()` is a correct cross-core broadcast under smp-shared. | Repro harness: `scratchpad/sshd_crash_hunt.py` (reboots SMP=4, fork-hammers, captures the fault dump). Root-cause the zeroed callee-saved context: `lldb` on the `[WILD-DA]`, trace clone context-setup vs the scheduler's context save/restore under concurrency. |
