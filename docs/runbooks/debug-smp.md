...
| `SMP=4` works for several exchanges then **wedges under sustained network load** (e.g. meow hangs on ~turn 4, right after finishing a response — at the inter-turn socket-close/new-connect boundary) | The M5 coarse-BKL **contention residual** — distinct from the M5d blocking-wait freeze (which is fixed: individual recvs no longer hold the BKL). Suspect accumulation (fd/socket-table/thread-slot) or a not-yet-covered inter-turn wait, OR the scheduler/IRQ BKL contention not scaling past 2 cores. | Reproduce with a scripted `meow -c` loop under `GDB=1 SMP=4`; when it wedges, grep the boot log for the last syscall before the `[BKL] stuck` flood and whether `owner` is *fixed* (hard deadlock) or *cycling* (contention). Use `SMP=2` meanwhile. Not yet root-caused. |
