...
| Forked children (`/bin/sshd`, `/bin/busybox`, shells) `SIGSEGV` under SMP=4; `ssh` sees "connection reset / kex_exchange_identification" but kernel heartbeats continue | **Memory corruption in the fork/exec/process-creation path during the high-concurrency BRINGUP window** (userspace fault, kernel survives). Fires only in the first few seconds (secondaries onlining + herd forking every service); a settled instance survives 30×20 concurrent fork rounds. **Heterogeneous** across boots (⇒ corruption, not one bug): `x0..x19` zeroed / child user-PC = kernel addr (`rust_sync_el0_handler_inner`) / null-write in musl startup / `[DA-MISS] ppid=0 … 0 mmap_regions`. **Ruled out:** missing TLB flush (`flush_tlb_all` is a correct broadcast), the `get_saved_user_context` zeroed-GPR fallback (`[CTXBUG]` never fired), and capture-time kernel PC (`[CAPBUG]` never fired ⇒ corruption is *post*-capture). | Repro: `scratchpad/sshd_crash_hunt.py` (reboot + immediate fork-hammer catches boot-window crashes). Root cause is fork/exec/process-registration not being fully SMP-safe — audit the BKL-drop sites (execve ELF read, M5b file-fault block I/O) and process-table / `mmap_regions` / context-store locking for overlapping fork/exec/exit; consider serializing process creation. Same family as the coarse-BKL residual. |
