...
| SMP=1: whole kernel freezes (ticks stop, no panic, QEMU ~3 % CPU) after starting a second server from SSH while a listener is bound; SMP=4 unaffected (same RAM) | Waiter parked in `idle_halt` (yield-less `blocking_relax_net`) monopolized the only core preempt-disabled; ticks' wake-pass ran but `schedule_indices` never switched, no voluntary-SGI producer left — `docs/archive/SECOND_LISTENER_SMP1_FREEZE.md` §2a | **FIXED** (`2439994b`) — non-idle halters stay preemptible; regression probe: `nettest parkprobe` |
...
| `test_epoll_multi_poller_pipe` reports `woken=1` at SMP>=2, with `[BKL] stuck: owner=N waiter=M` immediately above it | Not epoll at all: the test's own `yield_now()` wait loop held the BKL, so the poller on a **peer** core could never enter the kernel to finish its lap. The poller sharing a core with the main thread always won — hence `woken=1`, never 0 | FIXED 2026-09-01 | `blocking_relax()` (drops the BKL around a WFI) in every wait in the test. [`../archive/EPOLL_MULTI_POLLER_PIPE_FLAKE.md`](../archive/EPOLL_MULTI_POLLER_PIPE_FLAKE.md) §9 |
