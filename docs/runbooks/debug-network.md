...
| SMP=1: whole kernel freezes (ticks stop, no panic, QEMU ~3 % CPU) after starting a second server from SSH while a listener is bound; SMP=4 unaffected | Undiagnosed; freeze lands in `execve`/port-conflict `bind` after a fork+exec+bind — see `docs/archive/SECOND_LISTENER_SMP1_FREEZE.md` | OPEN | GDB at SMP=1 (deterministic trigger), §5 of that doc |
