...
| `nslookup`: *Bad file descriptor* | `write()` on a connected UDP socket. **Do not read the old "DNS itself works" note here as reassurance** — on 2026-09-18 the reverse held: `nslookup` resolved perfectly while `curl` and `git` could not resolve at all, because they use **c-ares**, which `connect()`s its UDP socket where busybox and musl do not. Three resolvers, three syscall sequences; a pass from one says nothing about the others. Fixed (`send` on connected UDP answered `EBADF`; `getsockname`/`getpeername` answered `ENOSYS`) — [`../archive/AKUMA_AMD64_DNS_CONNECTED_UDP.md`](../archive/AKUMA_AMD64_DNS_CONNECTED_UDP.md) |
...
| the whole machine dies mid-build — no ping, no ssh, and the last thing on the framebuffer is a truncated `[SWIT` | `[SWITCH FREED-CR3]`: a task slot's `space_root` named a page-table root the PMM already had back, and the switch installed it. **ROOT-CAUSED and FIXED 2026-09-18.** The free gate's saved-context arm was dead on this target — the x86 probe was registered into `akuma-threading` but `akuma-mmu` asks through a `SchedHooks` table that only `akuma_exec::init` registers, and **amd64 never calls it**, so the arm degraded to `None` on both the free and the drain path. Fixed by registering the table in `sched::register_hooks`. The tripwire is now survivable too: one `StackWriter` + a single flush, and the slot is demoted to the kernel root instead of the `mov cr3` that triple-faulted the box, so the next occurrence is readable over ssh rather than on a camera. Tell, if it comes back: a non-zero `debug: no switch installed a freed page-table root` in the boot tally. [`../archive/AKUMA_AMD64_SWITCH_FREED_CR3_UAF.md`](../archive/AKUMA_AMD64_SWITCH_FREED_CR3_UAF.md) |
