...
| A `pthread_create`d thread dies `SIGSEGV after 0.00s` at a **fixed** `ELR` with near-null `FAR`, preceded by `sig 11 needs sigaltstack but slot N has none — re-pending`, and the group is killed by `SIGSEGV in clone_thread` | **OPEN.** No longer *blocks* the self-host build (retrying gets past it, 2026-08-05) but still costs a whole crate compile each time. The child reads the first 8 bytes of its own clone argument and gets stale/zero content: [`runbooks/debug-thread-spawn-segv.md`](runbooks/debug-thread-spawn-segv.md). The `pthread_join` waiters it leaves behind look exactly like a futex lost wakeup and are not one. **Read §3a first** — the fault dump now prints the clone hand-off as the parent wrote it vs as the child reads it, plus `ttbr0_live` vs `ttbr0_proc`; those three flags pick the theory, so a repro that ignores them is wasted |
...
| Two `SIGSEGV in clone_thread` faults in **different pids** with byte-identical `x19`/`SP_EL0`/`FAR` | Either no-ASLR determinism (what earlier sessions assumed) or the two processes are reading the *same memory*. The second reading is the current lead theory and unifies this with the "victim running in another pid's address space" class: [`runbooks/debug-thread-spawn-segv.md`](runbooks/debug-thread-spawn-segv.md) §2d, §3c T1 |
