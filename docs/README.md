...
| A thread faults and the dump says `*** AS MISMATCH: L0 BASE DIFFERS ***` | **Candidate fix landed 2026-08-06, pending `-j4` proof.** The core is executing that thread in a *third* process's page tables, so every "corrupt pointer" in the dump is a correct pointer resolved in the wrong space: [`runbooks/debug-thread-spawn-segv.md`](runbooks/debug-thread-spawn-segv.md) §2f. The mechanism found: lock-free check-then-store wake/publish races on `THREAD_STATES` could stamp READY onto a recycled or half-built slot, resuming the previous occupant's context (§2g). A bare **ASID** difference is *not* a finding — a cloned thread runs under the parent's ttbr0 while its `Process` gets a fresh ASID over the same L0 |
...
| A freshly-exec'd binary dies instantly at `ELR = N × 0x30000000 + 0x6c964`, N climbing by 1 per fault across the boot | **ROOT-CAUSED AND FIXED 2026-08-06.** Not an exec at all — a **fork** child launched at its *parent's* ELF entry point. `get_saved_user_context`'s fallback returned the slot's `user_entry`/`user_sp` triple whenever no live EL0 trap frame was published; that triple records where the image was **first entered**, which for a dynamically linked parent is ld-musl's `_dlstart`. The child re-ran musl's RELR `*slot += base` loop over an already-relocated interpreter page — one `+= base` per birth on one physical word. The tell is `SP_EL0`: constant **per parent** and independent of the child's argv. The fallback is gone; the syscall now fails loudly instead. [`runbooks/debug-thread-spawn-segv.md`](runbooks/debug-thread-spawn-segv.md) §2h. Regress with `threading::state_transition_guard_tests::a_thread_with_no_trap_frame_never_yields_a_child_context` |
| `[NO-TRAPFRAME] refusing child of tid=…` in the log, or `[syscall] clone: fork failed: No saved context` | The §2h guard firing: a fork/vfork/clone found no live EL0 trap frame and refused to invent a child context. The syscall fails with `ENOMEM` — loud, local, recoverable. Compare `tid` and `cur` on the line: differing means a `current_thread_id()` mismatch between publish and read, matching means something cleared `CURRENT_TRAP_FRAME`. This is the open sub-question of [`runbooks/debug-thread-spawn-segv.md`](runbooks/debug-thread-spawn-segv.md) §2h |
