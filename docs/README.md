...
| Nightly `cargo` dies instantly under HVF with `[Exception] Unknown from EL0: EC=0x0` at a **constant** `ELR`, ~71 syscalls into startup | **OPEN.** One undefined instruction in that one binary (nightly `rustc` and apk `cargo` are fine; `HVF=0` avoids it). `EC=0x0` is an undefined instruction, **not** the `CNTP` trap the old note claimed — that would be `EC=0x18`. Fixing it restores the nightly-cargo fingerprint cache: [`runbooks/selfhost-kernel-build.md`](runbooks/selfhost-kernel-build.md) §6 |
