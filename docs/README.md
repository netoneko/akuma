...
| In-VM build sits on `Compiling akuma v0.0.7` forever at `-j4`; waiters on `0x300c2340` with `queued_for` > 900 s and **no** `[kill]`/`[Fault]` lines in the whole boot | **OPEN**, but has a workaround: build that crate at **`-j1`** (68 s). Not the cross-process key collision — every orphan is one tgid: [`runbooks/debug-futex-lost-wakeup.md`](runbooks/debug-futex-lost-wakeup.md) §4a + [`runbooks/selfhost-kernel-build.md`](runbooks/selfhost-kernel-build.md) §5.1 |
