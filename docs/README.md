...
| A freshly-exec'd **dynamic** binary dies instantly at `ELR = N × 0x30000000 + 0x6c964`, N climbing by 1 per fault across the boot — or the same `ELR` with `FAR` decoding as **ASCII** (`"libder-8"`, `"+outline"`) | **OPEN** — the ld-musl startup class: `0x6c964` is the indirect `__dls2` call. `e_entry`, `AT_BASE` and "relocations applied twice" are all **ruled out**: [`runbooks/debug-thread-spawn-segv.md`](runbooks/debug-thread-spawn-segv.md) -> "A second, separate crash". N advances per *fault*, not per exec, so the carrier is shared mutable state. Also ruled out (2026-08-06, §2c): **no kernel relocation writer can produce the accumulation** — all three appliers use `*ptr = base + addend` and `INTERP_BASE` is a `const`, so the carrier is musl's own `GETFUNCSYM` slot with a second writer |
