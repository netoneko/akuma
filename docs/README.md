...
| A freshly-exec'd binary dies instantly at `ELR = N × 0x30000000 + 0x6c964`, N climbing by 1 per fault across the boot — or the same `ELR` with `FAR` decoding as **ASCII** (`"libder-8"`, `"+outline"`) | **OPEN — now the `-j4` blocker, and cornered (2026-08-06).** The `+= base` writer is musl's **RELR** apply loop (`_dlstart+0x12c`; Alpine's ld-musl carries `DT_RELR`, implicit-addend by design; the kernel has no RELR handling). N is global across *different binaries* (rustc, cargo, busybox.static), so the accumulating slot lives on ONE shared physical frame. The slot is now pinned exactly: ld-musl vaddr `0xc03b8`, link-time value `0x6c964`. The file-page-cache theory is **falsified** (`[FPCACHE] entries=0` for the whole 1521-fault run). 1521 faults resolve to just **two** `(ttbr0, slot PA)` pairs, so victims share a long-lived address space in groups. Sharpest lead: `/bin/busybox{,.static}` are static PIEs with **no `PT_INTERP`**, so they should have nothing mapped at `0x30000000` at all. `[RELR]` forensics ship in the abort handler: [`runbooks/debug-thread-spawn-segv.md`](runbooks/debug-thread-spawn-segv.md) → "What the instrumented runs settled" |
