...
| A thread parked forever in an unreturned `futex` (`sc=-1 tsc=98` in `[THR-DUMP]`), low CPU, frozen syscall count | Five measured futex divergences from Linux, chiefly **requeued waiters never removed from the requeue target** (each stale entry ate one future wake) — **FIXED**: [`archive/FUTEX_REQUEUE_LOST_WAKEUP.md`](archive/FUTEX_REQUEUE_LOST_WAKEUP.md). Current behaviour: [`reference/subsystems/syscalls/sync.md`](reference/subsystems/syscalls/sync.md). Reproduce/regress with `userspace/forktest/c_stress/futexops.c` — run the same binary under `docker --platform linux/arm64` to see the Linux baseline (5 PASS either side) |
