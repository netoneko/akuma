...
if [ "$DISK" != "none" ]; then
    if [ -z "$DISK" ]; then
        ...
        # 128, not the old 32: matches `mkdisk.sh`'s own default, sized for a
        # real `apk add` dependency chain (32 MiB ran to `ENOSPC` partway
        # through installing `curl`'s 14 packages) since 2026-09-04 — see that
