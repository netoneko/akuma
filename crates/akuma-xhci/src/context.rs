...
/// The Add-Context bitmap for a **Configure Endpoint** command: `A0` plus one
/// bit per endpoint DCI, and never `A1`.
///
/// EP0 belongs to Address Device and Evaluate Context. A Configure Endpoint
/// that also claims it is rejected — QEMU's controller requires the low two bits
/// of the Add flags to be exactly `0b01` and answers `TRB Error` (cc=5)
/// otherwise, and real controllers agree. That completion code names the TRB,
/// not the flag, so the symptom is a bring-up that enumerates the device, reads
/// its descriptors, finds its bulk endpoints and then fails on the last command
/// before the disk works, with nothing pointing at the cause.
///
/// A0 is always set: Configure Endpoint is also what updates the slot context's
/// Context Entries to cover the new endpoints.
///
/// This exists as a builder rather than a rule in a comment because the call
/// site is a chain of `add_flag(..) | add_flag(..)` where `add_flag(1)` looks
/// exactly as reasonable as the others — it is what Address Device just used.
#[must_use]
pub fn configure_endpoint_add_flags(endpoint_dcis: &[u8]) -> u32 {
    let mut add = add_flag(0);
    for &dci in endpoint_dcis {
        debug_assert!(dci >= 2, "DCI 0 (slot) and 1 (EP0) are not endpoints to configure");
        if dci >= 2 {
            add |= add_flag(dci);
        }
    }
