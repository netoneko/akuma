...
/// The Add flags for a Configure Endpoint command must set `A0` and leave `A1`
/// clear — the bug that stopped the first working bring-up one command short.
///
/// The controller's answer to `A1` being set is `TRB Error` (cc=5) against the
/// command, which says nothing about a flag. QEMU's `qemu-xhci` requires the
/// low two bits to be exactly `0b01`; that is what this pins.
#[test]
fn configure_endpoint_add_flags_exclude_ep0() {
    use akuma_xhci::context::{configure_endpoint_add_flags, dci};

    // The reference enclosure: bulk IN 0x81 (DCI 3), bulk OUT 0x02 (DCI 4).
    let add = configure_endpoint_add_flags(&[dci(0x81), dci(0x02)]);

    assert_eq!(add & 0b11, 0b01, "A0 set, A1 (EP0) clear — the whole rule");
    assert_eq!(add, (1 << 0) | (1 << 3) | (1 << 4));

    // And it survives the trip through the Input Control Context, which is where
