//! The kernel's compile-time tunables.
//!
//! Was `src/config.rs`. It became a crate on 2026-09-01 so that **crates can
//! fold against these consts instead of being handed them at runtime**.
//!
//! # Why not runtime config
//!
//! Four crates had already grown a `…Config` struct and a shim in the binary to
//! populate it (`akuma-fpcache`, `akuma-vfs-glue`, `akuma-syscalls-log`,
//! `akuma-syscalls-ipc` — 141 lines of shim between them), and `src/syscall/`
//! would have needed a fifth with 26 fields. That pattern has a cost that grows
//! with every extraction: **a `const` that becomes runtime config stops
//! const-folding whatever it gates.** Thirteen of the flags `src/syscall/` reads
//! are unconditionally `false` and guard trace blocks on the syscall path;
//! handing those over would have retained all of that code in every profile.
//! Two such flags in `akuma-vfs-glue` already cost a page of `extreme-size`.
//!
//! A crate of `pub const`s folds across the LTO boundary exactly as it did
//! in-crate, and there is still only one definition of each value.
//!
//! # Why not `akuma-primitives`
//!
//! Size and charter. `akuma-primitives` is ~860 code lines of *mechanism* — IRQ
//! masking, per-CPU registers, the console writer, the clock, errno, addresses.
//! This is ~1,200 lines of *policy*, and merging would make policy half of that
//! crate. Everything depends on `akuma-primitives`, so a tunable edit would also
//! rebuild the tree; here it rebuilds only actual consumers.
//!
//! # Its one dependency
//!
//! `MAX_THREADS`, re-exported from `akuma_primitives::preempt`, where it is
//! defined because every per-slot static is indexed by it. Everything else here
//! depends on nothing.
//!
//! # Adding a const
//!
//! Put it here, not in a consumer. If it needs a profile gate, check that
//! `build.rs` forwards the cfg — an unforwarded cfg silently selects the wrong
//! arm. And do **not** re-add `#![allow(dead_code)]`: it is what let fourteen
//! tunables for the built-in SSH server and `ps` builtin (removed 2026-08-10)
//! sit here for three weeks after their last reader went away.
...
/// `execve`/`spawn`.
///
/// Linux's `MAX_ARG_STRLEN` is 32 pages (128 KB); cargo/rustc routinely pass
/// multi-KB single arguments (e.g. smoltcp's build-script `--check-cfg
/// 'cfg(feature, values(...))'` is ~5 KB), so the self-host build needs the
/// full Linux cap. The small-memory profiles keep a tight cap to bound the per-
/// arg heap copy — they never run a host toolchain.
///
...
/// Per-thread futex queue history, and orphaned-parked-thread detection.
///
...
/// Prefer never-scheduled READY threads over wakeup locality.
///
...
/// hint and the normal round-robin scan.
///
/// This is a falsification test, not a real fix: if flipping it on makes the
/// `jobserver_stress` barrier hang under `SMP=4` + CPU-hog contention
/// disappear, some newly-`clone()`d threads can be starved indefinitely by the
/// normal scheduler under this load pattern — a thread-admission/fairness bug,
/// not the futex wake-loss bug the rest of that investigation assumed. If the
/// hang persists unchanged, this rules the theory out cleanly. Bypasses
/// `round_robin_idx` bookkeeping on the fast path, so it is not itself fair —
/// leave `false` outside this experiment.
...
/// Boot-time SNTP, for platforms with no RTC.
///
...
/// platform with no RTC (Firecracker).
///
/// Best-effort and non-fatal either way — see `src/ntp_boot.rs`.
pub const ENABLE_NTP_BOOTSTRAP: bool = true;

/// NTP server the boot-time fallback queries — an IP literal, deliberately, not
/// a hostname.
///
/// `akuma_net::smoltcp_net::dns_query` fast-paths an IP literal without any DNS
/// lookup, which matters because the one platform this fallback exists for has
/// broken DNS: `overlays/devbox-firecracker`'s README documents that
/// `QEMU_DNS_SERVER` (`10.0.2.3`, smoltcp_net.rs) is SLIRP's forwarder address,
/// hardcoded, and nothing listens there under Firecracker (`guest-setup.sh`'s
/// dnsmasq answers on `10.0.2.2` instead) — a hostname here would just time out
/// on the exact platform that needs this fallback most. `216.239.35.0` is
/// Google Public NTP's anycast address (`time.google.com`), stable since ~2014.
pub const NTP_SERVER_HOSTNAME: &str = "216.239.35.0";

/// How long the boot-time SNTP round trip waits for a response before giving
/// up.
///
/// Runs pre-IRQ-unmask on a busy-poll loop, so this is real boot latency on any
/// platform that hits it — kept short since a timeout is not fatal.
...
/// 1000 ticks — and every **100** while any fork is in progress.
///
/// Left over from the fork-wedge investigations; `[Heartbeat] … SmolNet Active`
/// already gives liveness, and under `-j4` a fork is almost always in progress,
/// so the ramped rate is pure noise on the serial line.
...
/// Enable the `vfork` fast-path (docs/COW_OPTIMIZATIONS.md Fix B).
///
/// When true, a `CLONE_VFORK` child SHARES the parent's address space (no CoW
/// copy, no demote) instead of routing through the full `fork_process`
/// replication — the parent is suspended until the child execs/_exits, so
/// sharing is safe and the child's immediate `exec` discards the shared view
...
/// [`FPCACHE_INFLATE_HEADROOM_MULT`]).
///
/// `20` → up to RAM/8 × 1.2, i.e. 614 MB at `MEMORY=4096`.
///
...
/// Verify every file-page-cache hit against disk. **Diagnostic, OFF by default.**
///
/// On every `file_page_cache` *hit*, re-read the page
...
/// Eager/lazy threshold for **anonymous private** `mmap`
/// (docs/COW_OPTIMIZATIONS.md, "lazy/zero-on-demand population").
///
/// An anonymous mapping of more than this many pages is registered as a lazy
/// region and demand-paged (zero-fill on first touch) instead of eagerly
/// allocating + zeroing + mapping every page in the syscall.
///
...
/// (see `compute_heap_size` in `src/main.rs`).
///
/// Set a fixed value to pin the heap — useful for squeezing onto very small
/// machines or reproducing a layout.
///
/// The auto heuristic already scales down for RAM < 256 MB, so an override is
/// rarely needed.
pub const KERNEL_HEAP_SIZE_MB: usize = 0;

/// Clamp (MiB) on the RAM figure the kernel's own reserves are computed from.
///
/// Applies to the kernel's *own*
...
/// Floor (bytes) on the `code_and_stack` region.
///
/// Guarantees the kernel binary
...
/// Slack (bytes) reserved ABOVE the boot-stack top, before the heap.
///
/// The boot
...
/// RAM floor (MiB) below which the resource-heavy boot self-tests are skipped.
///
/// Below this detected-RAM threshold (MiB), skip the resource-heavy boot self-
/// tests (parallel multi-process / FP-across-preemption) that need to spawn
/// several processes at once — they can't fit on tiny machines and would halt
/// the boot.
///
/// Core tests still run. `0` disables the skip (always run everything).
///
...
///
/// Each entry is 32 B, so this caps the ring buffer at `N × 32 B` of heap per
/// live/recently-dead process. 64 keeps the last ~64 syscalls — enough to see
/// the lead-up to a fault — for ~2 KB/process (was 500 → ~16 KB/process, far
/// more history than debugging needs). Only allocated when
/// `PROC_SYSCALL_LOG_ENABLED` (off on extreme).
...
/// Log **`read()`** on **pipe reader FDs** (`PipeRead`): pid, tid, fd, pipe id,
/// user buffer pointer and count on each syscall, plus **`validate_user_ptr`**
/// / **`copy_to_user`** failures.
///
/// Uses **`tprint`** timestamps so serial correlates with **`[signal]`** / mmap
/// lines (`GO_FORKTEST_DEBUG.md`, parent Pattern 2).
///
...
/// Log `x8`/`x0`–`x5` for a SIGSEGV inside Go's shared syscall trampoline.
///
/// When **`true`**, a fatal SIGSEGV whose **`ELR_EL1`** falls in the inclusive
/// range **`DEBUG_SIGSEGV_SYSCALL_STUB_ELIR_MIN`..=**
/// **`DEBUG_SIGSEGV_SYSCALL_STUB_ELIR_MAX`** logs **`[sigsegv-syscall]`** with
/// **`x8`** (syscall number) and
/// **`x0`–`x5`** — disambiguates **`read`** vs **`epoll_ctl`** vs other
/// syscalls when Go reports **`PC≈0x13060`** (shared syscall trampoline).
///
/// See **`docs/GO_FORKTEST_DEBUG.md`** (Pattern 2, Agent handoff).
///
...
/// Trace every edge-triggered (`EPOLLET`) readiness decision.
///
/// One line per fd in `sys_epoll_pwait`:
...
/// `process::reclaim_retired_processes` frees it.
///
/// Must outlast any BKL-dropped window (no-bkl-vfs/no-bkl-mm/no-bkl-process)
/// that could still hold a raw pointer to it — see `unregister_process`'s doc
/// comment and docs/archive/BKL_PHASE7E_PROCESS_TABLE_RECLAIM.md. Same order of
/// magnitude as THREAD_CLEANUP_COOLDOWN_US since the windows being outlasted
/// are the same kind.
