//! Synchronous filesystem facade over this crate's mount table.
//!
//! Was `src/fs.rs`. It moved in with the layer it delegates to on 2026-09-01:
//! it is a thin sync wrapper over `crate`'s mount table and namespaces, and
//! keeping it up in the binary meant `src/syscall/` reached 13 symbols across a
//! crate boundary for functions whose whole body is a `with_fs` call.
//!
//! The binary re-exports it as `crate::fs`, so no call site changed.
...
pub use crate::{DirEntry, FsError};
...
pub fn init() -> Result<(), FsError> {
    ...
    // See docs/archive/BKL_RUSTC_SCALING_BASELINE.md.
    {
        ...
        crate::fpcache_init(ram_bytes);
    }

    // Mount ext2 filesystem at root
    let ext2_fs = crate::ext2::mount()?;
    crate::mount_with("/", Some("/dev/vda"), 0, ext2_fs)?;

    log("[FS] Ext2 filesystem mounted at /\n");

    // Mount procfs at /proc
    let proc_fs = alloc::sync::Arc::new(crate::proc::ProcFilesystem::new());
    crate::mount_with("/proc", Some("proc"), 0, proc_fs)?;

    log("[FS] Procfs mounted at /proc\n");

    // Verify by listing root directory
    match crate::list_dir("/") {
        Ok(entries) => {
            log("[FS] Root directory accessible\n");
            log("[FS] Files in root: ");
            akuma_primitives::safe_print!(32, "{}\n", entries.len());
        }
        Err(e) => {
...
pub fn read_file(path: &str) -> Result<Vec<u8>, FsError> {
    ...
    match crate::read_file(path) {
        Ok(data) => {
            if crate::cfg_syscall_debug_io_enabled() {
                akuma_primitives::safe_print!(128, "[FS] read_file(\"{}\") -> {} bytes\n", path, data.len());
            }
            Ok(data)
        },
        Err(e) => {
            if crate::cfg_syscall_debug_io_enabled() {
                akuma_primitives::safe_print!(128, "[FS] read_file(\"{}\") -> Error: {}\n", path, e);
            }
            Err(e)
        }
    }
...
pub fn read_at(path: &str, offset: usize, buf: &mut [u8]) -> Result<usize, FsError> {
    ...
    crate::read_at(path, offset, buf)
}

/// Read for an open file description — by the inode `open(2)` resolved, when it
/// resolved one. See [`crate::read_at_open_file`].
pub fn read_at_open_file(path: &str, mount_id: u32, inode: u32, offset: usize, buf: &mut [u8]) -> Result<usize, FsError> {
    if !is_initialized() {
        return Err(FsError::NotInitialized);
    }
    crate::read_at_open_file(path, mount_id, inode, offset, buf)
}

/// `stat` for an open file description — by the inode `open(2)` resolved, when
/// it resolved one. See [`crate::metadata_open_file`].
pub fn metadata_open_file(path: &str, mount_id: u32, inode: u32) -> Result<crate::Metadata, FsError> {
    if !is_initialized() {
        return Err(FsError::NotInitialized);
    }
    crate::metadata_open_file(path, mount_id, inode)
}
