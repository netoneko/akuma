...
mod imp {
    ...
    pub struct VirtioSoundDevice {
        inner: UnsafeCell<VirtIOSound<VirtioHal, SteppedMmioTransport<'static>>>,
        out_stream: u32,
    ...
    impl VirtioSoundDevice {
        ...
        fn inner_mut(&self) -> &mut VirtIOSound<VirtioHal, SteppedMmioTransport<'static>> {
    ...
    pub fn init() -> Result<(), AudioError> {
        ...
        let device = probe::probe_with(probe::device_id::SOUND, |i, transport| {
            ...
            let Ok(mut snd) = VirtIOSound::<VirtioHal, SteppedMmioTransport<'static>>::new(transport) else {
