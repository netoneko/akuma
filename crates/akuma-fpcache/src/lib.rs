...
/// Re-exported so `safe_print!` resolves unqualified throughout this crate —
/// the same move `akuma-exec` makes, and what let the moved call sites keep
/// their original spelling. (`tprint!` lives in `src/console.rs` and does not
/// travel; the two differ only in the timestamp prefix.)
...
/// Release one reference to `pa`, attributed to `site`.
///
/// This is `src/pmm.rs::free_page_at` inlined: that shim exists only to supply
/// the current tid and convert a `PhysFrame`, neither of which needs `src/`.
fn free_frame(pa: usize, site: akuma_pmm::FreeSite) {
    akuma_pmm::free_page_at(pa, akuma_primitives::preempt::current_tid() as u32, site);
}
...
/// Evictions forced to take a **still-mapped** entry.
///
/// The bounded scan found no unmapped one. Each costs the next mapper a
/// `read_at` while freeing nothing, so
...
/// the base cap.
///
/// [`config::FPCACHE_INFLATE_PCT`]: FpcacheConfig::inflate_pct
...
///
/// [`config::FPCACHE_INFLATE_HEADROOM_MULT`]: FpcacheConfig::inflate_headroom_mult
pub fn reassess_cap() {
    if !enabled() {
        return;
    }
...
pub fn lookup_and_ref(
    ...
    if !enabled() || inode == 0 || mount_id == 0 {
        return None;
    }
    let hit = with_irqs_disabled(|| {
...
pub fn insert(mount_id: u32, inode: u32, file_off: usize, frame: PhysFrame, icache_done: bool) {
    ...
    let evicted = with_irqs_disabled(|| {
            ...
            .take(EVICT_SCAN)
        {
            ...
            if akuma_pmm::cow_ref_get(v.pa) <= 1 {
                unmapped = Some((k, v));
                break;
            }
        }
    ...
    if let Some((pa, took_mapped)) = evicted {
        ...
        free_frame(pa, akuma_pmm::FreeSite::FpcacheEvict);
    }
}
...
pub fn invalidate_inode(inode: u32) {
    ...
    let dropped: alloc::vec::Vec<usize> = with_irqs_disabled(|| {
        let mut pages = PAGES.lock();
    ...
    for pa in dropped {
        free_frame(pa, akuma_pmm::FreeSite::FpcacheInvalidate);
    }
}
...
#[must_use]
pub fn len() -> usize {
    with_irqs_disabled(|| PAGES.lock().len())
}
...
pub fn shrink(want: usize) -> usize {
    ...
    let dropped: alloc::vec::Vec<usize> = with_irqs_disabled(|| {
        ...
        let victims: alloc::vec::Vec<(u32, u32, usize)> = pages
            ...
            .filter(|(_, e)| akuma_pmm::cow_ref_get(e.pa) <= 1)
            .map(|(k, _)| *k)
            .take(want)
            .collect();
...
///
/// Writes into the caller's buffer instead of returning a `String` — this runs
/// on the periodic memory-monitor tick and shouldn't need a healthy heap to
/// report itself.
