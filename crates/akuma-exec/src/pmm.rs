...
//!
//! Was `src/pmm.rs`. It moved here on 2026-09-01 because the types it converts
//! — [`PhysFrame`] and [`FrameSource`] — are this crate's, and because
...
//! `crate::process::reclaim` the same day. What the exception path needs
//! from here (`dp_counters_line`) it takes through `ExceptionHooks`.

pub use crate::{FrameSource, PhysFrame};
...
// `crate::process::reclaim`, which reads `akuma_pmm` directly.
...
// `crate::process::reclaim`, which registers the bridge hook from
// `crate::init`. The exception path reaches `report_poison_value` through
